...
matchToConfiguration()
{
    ...
    then
        PKGS="\
$SYSTEM $NETWORKMANAGER $MONITORING $MICROCODE $AMDGPU $AMDGPGPU $AUDIO \
...
$MULTIMEDIA $STEAM $HASKELL $PYTHON $LATEX $DEVUTILS $ODD"
    ...
    then
        PKGS="\
$SYSTEM $MICROCODE $AMDGPU $NETWORKMANAGER $AUDIO $BLUETOOTH $KDE $FONTS \
$BROWSERS $OFFICE $DISCORD $STEAM $LUTRIS $MINECRAFT"

    elif [ $(echo $1 | grep --ignore-case -q "htpc"; echo $?) -eq 0 ]
    then
        PKGS="\
$SYSTEM $MICROCODE $AMDGPU $NETWORKMANAGER $AUDIO $BLUETOOTH $KDE $FONTS \
$BROWSERS $OFFICE $STEAM $VPN $BITTORRENT $NVIM $MUSIC $MULTIMEDIA"
